...
LINE: while (<HPASM>) {
    ...
    } elsif (my ($elevel, $edesc) = /^(INFO|REPAIRED|CAUTION|CRITICAL):(.*)$/) {
    ...
    } elsif (/^The IML Log is empty./) {
	#don't work, return code 3 (unkown)
	#$iml_worst_event{level} = 'NONE';
	#empty iml log = all is good, I think ?
	$iml_worst_event{level} = 'INFO';
    } else {
